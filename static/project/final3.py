...
print(os.getcwd())
from static.project.CamScanner import Camera


class RoomSegmentation:
    def __init__(self, m=1, b=5, min_dist=1600, min_l=100, image_name=None, image_file=None, base_dir=None,
                 media_upload=None):
        self.m_thresh = m
        self.b_thresh = b
        self.l_thresh = min_dist
        self.first_line_tresh = min_l
        if base_dir is None:
        ...
        if base_dir is None:
            if image_name is None:
                if 'Camera' not in os.listdir('./results'):
                    os.mkdir("./results/Camera")
                self.dir = "./results/Camera"
            else:
            ...
            if image_file is None:
                self.img = cv2.imread(media_upload, 0)
                self.img = self.convert_to_bw(self.img, 200)
            if image_name.split('.')[0] not in os.listdir(base_dir):
                os.mkdir(os.path.join(base_dir, image_name.split('.')[0]))
            self.dir = os.path.join(base_dir, image_name.split('.')[0])
    ...
    def convert_to_bw(self, image, thresh):
    ...
    def line_segmentation(self, img, name):
        ...
        for line in lines:
            ...
            x2 = line[0][2]
            y2 = line[0][3]
            if (x2 - x1) ** 2 + (y2 - y1) ** 2 < self.first_line_tresh:
                continue
            if x2 - x1 == 0:
                continue
            m = (y2 - y1) / (x2 - x1)
            b = y2 - m * x2
            detectedLines.append([m, b, (x1, y1), (x2, y2)])
    ...
    def draw_lines(self, img, lines, name):
        tmp = cv2.createLineSegmentDetector(1).drawSegments(img, lines)
        cv2.imshow(name, tmp)

    def connected_component(self, img):
    ...
    def rotate_imge(self, img, theta):
        rows, cols = img.shape
