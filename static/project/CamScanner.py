...
class Camera:
    ...
    def detect_map(self):
        ...
        # img = cv2.imread("./Data/test.jpg", 1)
        ...
        real = cv2.warpPerspective(self.img, m, (width, length))

        binary = cv2.cvtColor(real, cv2.COLOR_BGR2GRAY)
        binary = cv2.adaptiveThreshold(binary, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 101, 10)
